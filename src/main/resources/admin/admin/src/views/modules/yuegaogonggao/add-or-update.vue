<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="要求" prop="yaoqiu">
          <el-input v-model="ruleForm.yaoqiu" 
              placeholder="要求" clearable  :readonly="ro.yaoqiu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="要求" prop="yaoqiu">
              <el-input v-model="ruleForm.yaoqiu" 
                placeholder="要求" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirongxiangqing = this.ruleForm.neirongxiangqing.replace(reg,'../../../springbootp68f1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yuegaogonggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yuegaogonggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
